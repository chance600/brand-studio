...
export const ImageStudio: React.FC<AgentProps> = ({ activeCampaign, history, onAssetGenerated }) => {
  ...
  const recentImages = history.filter(a => a.type === 'image');

  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-bold text-white tracking-tight">Creative Studio</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 bg-slate-900 rounded-xl p-6 border border-slate-800 min-h-[500px] shadow-xl">
            {/* GENERATE TAB */}
            {activeTab === 'generate' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 h-full">
                ...
                <div>
                    ...
                    <textarea 
                    className="w-full bg-slate-950 border border-slate-700 rounded-lg p-4 text-sm h-32 focus:ring-2 focus:ring-purple-500 outline-none"
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <select 
                        ...
                        className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-sm focus:ring-2 focus:ring-purple-500 outline-none"
                    >
                        {Object.values(AspectRatio).map(r => <option key={r} value={r}>{r}</option>)}
                    </select>
                    </div>
                    <div>
                    <label className="text-xs uppercase tracking-wider text-slate-500 mb-2 block font-bold">Size</label>
                    <select 
                        value={imageSize} 
                ...
                <button 
                    ...
                    className="w-full py-4 bg-purple-600 hover:bg-purple-500 rounded-lg font-bold text-white transition-all disabled:opacity-50 shadow-lg shadow-purple-900/30"
                >
                    {loading ? 'Generating...' : 'Generate 3 Pro Image'}
                </button>
                </div>

                <div className="bg-slate-950 rounded-xl border border-slate-800 flex items-center justify-center relative overflow-hidden min-h-[300px]">
                {generatedImg ? (
                    <img src={generatedImg} alt="Generated" className="max-w-full max-h-[450px] object-contain shadow-2xl rounded-lg" />
                ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="border-2 border-dashed border-slate-700 rounded-xl p-8 text-center hover:border-blue-500 transition-colors cursor-pointer relative bg-slate-950/50">
                    ...
                    {remixSource ? (
                    <img src={remixSource} className="h-48 mx-auto object-contain rounded" alt="Source" />
                    ) : (
                    <div className="space-y-2">
                        <span className="text-3xl block">📤</span>
                        <p className="text-slate-500 text-sm">Click to upload product image</p>
                    </div>
                    )}
                </div>
                
                <div>
                    <label className="text-xs uppercase tracking-wider text-slate-500 mb-2 block font-bold">Instructions</label>
                    <input 
                        type="text"
                        className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                ...
                <button 
                    ...
                    className="w-full py-4 bg-blue-600 hover:bg-blue-500 rounded-lg font-bold text-white transition-all disabled:opacity-50"
                    >
                    {loading ? 'Remixing...' : 'Remix with Flash Image'}
                    </button>
                </div>

                <div className="bg-slate-950 rounded-xl border border-slate-800 flex items-center justify-center min-h-[300px]">
                {remixedImg ? (
                    <img src={remixedImg} alt="Edited" className="max-w-full max-h-[450px] object-contain rounded-lg" />
                ) : (
                    <p className="text-slate-600 font-medium">Remixed result</p>
                )}
                </div>
            </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="border-2 border-dashed border-slate-700 rounded-xl p-8 text-center hover:border-emerald-500 transition-colors cursor-pointer relative bg-slate-950/50">
                    ...
                    {analyzeSource ? (
                    <img src={analyzeSource} className="h-48 mx-auto object-contain rounded" alt="Analyze Source" />
                    ) : (
                    <div className="space-y-2">
                         <span className="text-3xl block">🔍</span>
                        <p className="text-slate-500 text-sm">Click to upload image to analyze</p>
                    </div>
                    )}
                </div>

                <div>
                    <label className="text-xs uppercase tracking-wider text-slate-500 mb-2 block font-bold">Question (Optional)</label>
                    <input 
                        type="text"
                        className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-sm focus:ring-2 focus:ring-emerald-500 outline-none"
                ...
                <button 
                    ...
                    className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 rounded-lg font-bold text-white transition-all disabled:opacity-50"
                    >
                    {loading ? 'Analyzing...' : 'Analyze with Gemini Pro'}
                    </button>
                </div>

                <div className="bg-slate-950 rounded-xl border border-slate-800 p-6 overflow-y-auto max-h-[500px]">
                    {analysisResult ? (
                    <p className="text-slate-200 whitespace-pre-wrap leading-relaxed">{analysisResult}</p>
                    ) : (
        ...
        <div className="lg:col-span-1 space-y-4">
            <h3 className="text-slate-400 font-bold text-xs uppercase tracking-wider mb-2">Recent Images</h3>
            <div className="grid grid-cols-2 gap-2 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                {recentImages.length === 0 && (
                    <p className="col-span-2 text-center text-xs text-slate-600 py-4 italic">No images yet</p>
                )}
                {recentImages.map((asset) => (
                    <div key={asset.id} className="relative aspect-square rounded-lg overflow-hidden border border-slate-800 group cursor-pointer" onClick={() => activeTab === 'generate' && setGeneratedImg(asset.content)}>
                        <img src={asset.content} className="w-full h-full object-cover transition-transform group-hover:scale-110" alt="History" />
